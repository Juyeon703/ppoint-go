...
func (dbc *DbConfig) SelectSumSalesPointByCustomDate(startDate, endDate string) (*dto.SumSalesPointDto, error) {
	...
	err := dbc.DbConnection.QueryRow("SELECT IFNULL(SUM(Sales), 0) AS sumSale, IFNULL(SUM(IF(pay_type='카드', sales, 0)), 0) AS sumCard, IFNULL(SUM(IF(pay_type='현금', sales, 0)), 0) AS sumCash, IFNULL(SUM(add_point), 0) AS sumAddP, IFNULL(SUM(IF(pay_type='소멸', 0, sub_point)), 0) AS sumSubP FROM ppoint.revenue WHERE date_format(create_date, '%Y-%m-%d') BETWEEN ? and ?", startDate, endDate).
...
func (dbc *DbConfig) SelectSumSalesPointByMemberId(memberId int) (*dto.SumSalesPointDto, error) {
	var result dto.SumSalesPointDto
	err := dbc.DbConnection.QueryRow("SELECT IFNULL(SUM(Sales), 0) AS sumSale, IFNULL(SUM(IF(pay_type='카드', sales, 0)), 0) AS sumCard, IFNULL(SUM(IF(pay_type='현금', sales, 0)), 0) AS sumCash, IFNULL(SUM(add_point), 0) AS sumAddP, IFNULL(SUM(IF(pay_type='소멸', 0, sub_point)), 0) AS sumSubP FROM ppoint.revenue WHERE member_id=?", memberId).
		Scan(&result.SumSales, &result.SumCard, &result.SumCash, &result.SumAddP, &result.SumSubP)
	if err != nil {
		return &result, err
	}
